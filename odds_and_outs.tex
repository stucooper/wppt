...
I have \fourd\tred\ and I call a raise from the big blind, two of us
take the flop. Calling a raise with \fourd\tred\ is questionable bad
poker, but ignore that for now. The big blind has \Ah\Jc. The flop comes
...
is a brick and I'm still losing.
...
river. My full collection of outs is
\Kd\Qd\Jd\tend\nined\sevd\sixd\fived\twod\fives\fivec\fiveh\ . Any
other river and I've lost this hand unless I can bluff my way out of
it.

A non-Diamond Five on the river is now my preferred card to hit,
because it will look like a brick to the Enemy and he should call an
enormous raise with it. A Diamond on the river will still win me the
pot, but will be an obvious out for me that the Enemy will slow down
his betting on.

Now see what happens when I have the same cards and the same flop and
turn, but this time the big blind has \Ac\As. On the flop of
...
Stack Odds are the maximum amount you can win from a player, by
dividing the call by the amount of his stack (or your stack, whichever
is smaller). The most important stack odd is that 1 in 8 chance of
flopping a set. If you play a low pocket pair to a raise, you might
win the Enemy's entire stack if you flop your set, and fold to betting
on the flop when you miss. In that case you're talking a three card
flop with the stack odds of 8 to 1.

% FIXME: do some numbers here

\section{Exercises}

% Notice I get the chapter number from a LaTeX variable, not hard coded.

\arabic{chapter}.1 I have \Ad\Jd\ and call a player's all-in on a flop
of \Ac\Kc\fourd\ . The enemy has \Ah\Kh\ for top two pair. Describe
all the runouts that win this hand for me, and any runouts that result
in a split pot.

\arabic{chapter}.2 What are the rough percentage chances of making
runner-runner trips, with unpaired hole cards that haven't hit a pair
on the flop?

\arabic{chapter}.3 How often does a flopped set improve to Quads or a Full
House? \textit{Hint: This is two percentages added together. You
can hit on the turn. Or you can miss on the turn but hit on the
river. Add these percentages up for the answer.}
